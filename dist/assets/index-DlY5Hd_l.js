(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))d(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const u of t.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&d(u)}).observe(document,{childList:!0,subtree:!0});function m(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function d(e){if(e.ep)return;e.ep=!0;const t=m(e);fetch(e.href,t)}})();function B(o){o.innerHTML=`
        <div id="quiz-app">
            <div id="start-screen">
    ...
    `;const n=o.querySelector(".memory-game"),m=n.querySelector(".start-game"),d=n.querySelector(".board-size"),e=n.querySelector(".emoji-set"),t=n.querySelector(".game-board"),u=n.querySelector(".attempts"),h=n.querySelector(".restart-btn");m.addEventListener("click",()=>{p(n)});function p(i){const g=d.value,S=e.value;let l,v;g==="4x4"?(l=4,v=4):g==="2x2"?(l=2,v=2):(l=2,v=4);let y={fruits:["🍎","🍌","🍇","🍉","🥑","🍒","🍍","🥥"],animals:["🐶","🐱","🐭","🐹","🐰","🦊","🐻","🐼"],custom:["😀","😂","😎","😍","😡","😭","😱","🤔"]}[S].slice(0,l*v/2),L=[...y,...y];L=k(L),t.innerHTML="",t.style.display="grid",t.style.gridTemplateColumns=`repeat(${v}, 1fr)`,t.style.margin="0 auto";let f=[],q=[],x=0,E=0;L.forEach((s,a)=>{const w=document.createElement("div");w.classList.add("card"),w.dataset.symbol=s,w.dataset.index=a,w.innerHTML="?",w.setAttribute("tabindex","0"),w.addEventListener("click",()=>r(w)),t.appendChild(w)}),h.classList.remove("hidden");function k(s){return s.sort(()=>Math.random()-.5)}function r(s){f.length<2&&!f.includes(s)&&!q.includes(s)&&(s.innerHTML=s.dataset.symbol,f.push(s),f.length===2&&setTimeout(c,500))}function c(){const[s,a]=f;x++,u.innerText=`Attempts: ${x}`,s.dataset.symbol===a.dataset.symbol?q.push(s,a):(s.innerHTML="?",a.innerHTML="?"),f=[],q.length===L.length&&(i.querySelector(".status").innerText=`You won in ${x} attempts!`)}i.addEventListener("keydown",s=>{const a=i.querySelectorAll(".card");s.key==="ArrowRight"?E=(E+1)%a.length:s.key==="ArrowLeft"?E=(E-1+a.length)%a.length:s.key==="Enter"&&r(a[E]),a[E].focus()}),h.addEventListener("click",()=>{i.innerHTML="",I(i)})}}function T(o){o.innerHTML=`
        <div class="chat-app">
            <div id="chat-setup">
